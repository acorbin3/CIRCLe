...
def print_splits(all_domains, conditions, df_collection):
    ...
    for s in all_domains:
        df_skin_filter = df_collection[df_collection['fizpatrick_skin_type'] == s]
        skin_type_len = len(df_skin_filter)
        print(f"\t skin type {s} : {skin_type_len} ({skin_type_len / len(df_collection) * 100:.2f})")
        for c in conditions:
            condition_len = len(df_skin_filter[df_skin_filter['label'] == c])
